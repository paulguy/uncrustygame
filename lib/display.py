...
def _driver_key(info):
    ...
    if bytes(info.name) == b'metal' or \
       bytes(info.name) == b'direct3d11':
        ...
        priority = 0
    elif bytes(info.name) == b'direct3d12':
        # try to use direct3d 12 if it's available but for now prefer
        # direct3d 11, which is supposedly planned to be removed eventually.
        priority = 1
...
def initialize_video(title :str,
    ...
    envdrv = getenv('SDL_RENDER_DRIVER')
    if envdrv is None:
        driver = list()
        ...
        print("Allowing SDL to choose renderer because SDL_RENDER_DRIVER is set to {}.".format(envdrv))
